...
def gridOfCircles(ground_surface, bounding_box, in_interval=None,out_interval=None,min_eta_inc=np.radians(5),num_in_pts=None,num_out_pts=None, in_pts=None, out_pts=None):
    ...
    for i in in_pts:
        for o in out_pts:
            # Controlli sulla validità dei punti input e output
            # 1. I punti non devono essere coincidenti
            if np.isclose(i, o):
                continue
            
            # 2. Il punto di input deve avere quota maggiore del punto di output
            quota_in = ground_surface(i)
            quota_out = ground_surface(o)
            if quota_in <= quota_out:
                continue
