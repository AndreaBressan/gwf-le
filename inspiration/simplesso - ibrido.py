...
c = [20 , 'kPa'] # soil coesion
phi = [40 , 'deg'] # friction angle
...
bounds = ((soilsurface.loc['B'][0] , 3*A) ,
...
sds_out = [zero['x'][1], zero['x'][1] * np.tan(np.radians(SlopeAngle)) if zero['x'][1] > soilsurface.loc['A'][0] else soilsurface.loc['A'][1]]
...
plt.text(soilsurface.loc['B'][0], 1.5*SlopeHeigth , fsText , fontsize = 11 , color='red')
plt.text(soilsurface.loc['B'][0], 2*SlopeHeigth , fsText_griglia , fontsize = 11 , color = 'blue')
# Plot SF contours
# x = grid['xc']
# y = grid['yc']
...
"""
    Slip Surface
"""
# for i in sommario.index:
#     xa , ya , Ra  = (sommario.loc[i , 'xc'] , sommario.loc[i , 'yc'] , sommario.loc[i , 'R'])
...
#     """
#     slipboundary = [sommario.loc[i ,'sds_out'] , sommario.loc[i ,'sds_in']]
#     maskk = (Circ['xcirc'] < slipboundary[0]) | (Circ['xcirc'] > slipboundary[1]) | (Circ['ycirc'] >= SlopeHeigth + 0.1)
#     Circ = Circ.mask(maskk).dropna()
#     plt.plot(Circ['xcirc'], Circ['ycirc'], c = 'red', linewidth=1 , ls = '--')
#     plt.plot(xa , ya , marker = '.' , c = 'gray')
"""
 critical circle
"""
