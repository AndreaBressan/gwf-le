...
gOptions=GridOptions(
    in_pts=[1.*slope_base,1*dist_max+slope_base, 2*dist_max+slope_base, 3*dist_max+slope_base],
...
soil_properties=SoilProperties( 
    cohesion       = lambda x,y : 1*np.ones_like(x+y),
...
bounds = ((1*slope_base,4*dist_max+slope_base) ,
          (-3*dist_max,1/4*slope_base) , 
          (0., 90))
...
#    print(f'Eta = {np.degrees(zero[j].x[2]):.2f}, Eta_min = {np.degrees(computeEtaMinForSurface(ground_surface,bounding_box,zero[j].x[0],zero[j].x[1])):.2f}')
    ...
    print(f'Factor of safety: {zero[j].fun:.3f}')
    print(f'Stability Number: {1/zero[j].fun:.4f}')
    print(f'entry point: {zero[j].x[0]:.3f} , exit point: {zero[j].x[1]:.3f} , delta: {zero[j].x[2]:.3f}')
